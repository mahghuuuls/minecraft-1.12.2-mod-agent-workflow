...
- The second process to start rotates `latest.log` at its startup, and the first process's lines before that moment survive only in the rotated `<date>-<n>.log.gz`. Start the server first, wait for its `Done` line, then start the client; then the rotated file holds the complete server startup (mod load, diagnostics) and `latest.log` holds the interleaved session. Copy both. A client started while the server is still loading loses the server's startup evidence.
- An optional mod that carries Mixins (loaded through MixinBooter) may load in `runClient` and still be refused by `runServer` as a duplicate mod: the mixin loader adds the jar to the class loader by its relative `.\mods\` path and FML finds it again by its absolute path. Plan checks that need such a mod on a real server for a packaged instance (tier 4 or 5) rather than the development dedicated server, and say so in the campaign map before the session.
...
When dependency metadata changes, inspect both the source metadata and the processed metadata inside the distributable artifact. Verify the loader-specific relation accepts the approved minimum and representative later versions, rejects versions below the minimum, and contains neither an exact-only constraint nor an upper bound. Keep this separate from the exact artifact used by the development build.
