...
$requiredTemplateHeadings = [ordered]@{
    ...
    'setup/artifact-templates/change-intake.md' = @('Cycle Identity', 'Reported Defect Confirmation', 'Impact Analysis', 'Stage Impact Matrix', 'Proposed Route')
...
$requiredProcessText = [ordered]@{
    ...
    'guidelines/coding-standards.md' = @('## Complexity Management', 'deep modules', 'information leakage', 'durable regression check')
    ...
    'stages/6-implementation-plan.md' = @('## Verification Environment Plan', '**Test now:**', '**Defer:**', '**Waive:**', '## Complexity Management', '## Defect Regression Protection')
    'stages/7-implementation.md' = @('## Validation Environment Tiers', '## Generated Artifact Inspection', '## Small Follow-Up Path', '**Complexity argument**', 'pre-correction failure')
